...
A single process that:
  ...
  - reads/writes Google Sheets (sheets.py),
...
class AppState:
    def __init__(self):
        ...
        self.sheets = None
        self.sheets_error: Optional[str] = None
...
async def lifespan(app: FastAPI):
    ...
    await loop.run_in_executor(None, state.init_sheets)
...
# ---------------------------------------------------------------------------
# Background pump: drain worker events, hit Sheets, push to browser
# ---------------------------------------------------------------------------
...
async def _handle_event(event: dict):
    ...
    if state.sheets is None:
        await broadcast({"type": "error",
                         "message": f"Google Sheets not connected: {state.sheets_error}"})
        return
    ...
    if kind == "checkin_batch":
        ...
        result = await loop.run_in_executor(
            None, state.sheets.receive_shipment,
            epcs, item_type, building, po_number, vendor)
        # Stay armed on this shipment so more boxes can be swept in.
    ...
    if kind == "scan" and event.get("mode") == reader_mod.CHECKOUT:
        epc = event["epc"]
        result = await loop.run_in_executor(
            None, state.sheets.deliver_to_site, epc)
        await broadcast({"type": "checkout_result", **result})
        return

    if kind == "inventory":
        epcs = event.get("epcs", [])
        result = await loop.run_in_executor(None, state.sheets.record_inventory, epcs)
...
async def get_status():
    return {
        ...
        "sheets_connected": state.sheets is not None,
        "sheets_error": state.sheets_error,
...
async def set_mode(req: ModeRequest):
    ...
    mode = req.mode
    if mode not in (reader_mod.IDLE, reader_mod.CHECKIN,
                    reader_mod.CHECKOUT, reader_mod.INVENTORY):
