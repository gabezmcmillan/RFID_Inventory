 ...
 *
 * Split out of `session.ts` so the Next.js middleware (Edge Runtime) can import
 ...
 * can load. This module touches nothing but `process.env` (available in the
 * Edge Runtime), so it is safe to import from middleware.
 *
