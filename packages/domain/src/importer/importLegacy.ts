 ...
 * - Target writes use the {@link SqlDatabase} surface (Node Turso), with
 *   `SCHEMA_SQL` applied.
...
export async function importLegacy(
  ...
  const tagPlaceholders = tagColumns.map(() => "?").join(",");
  const tagRows = legacy.prepare("SELECT * FROM tags ORDER BY id").all() as Record<string, unknown>[];
  for (const r of tagRows) {
    const values = [
      r.id, r.epc, r.item_type, r.item_name, r.bol_number, r.po_number, r.building,
      r.sector, r.vendor, r.sku, r.mfc_date, r.quantity, r.remaining, r.status,
      r.received_at, r.delivered_at, r.checkout_building, r.flag, r.flagged_at,
      r.created_at, r.updated_at, tagHasBolDocId ? r.bol_doc_id : null,
    ];
    await target.run(
      `INSERT INTO tags (${tagColumns.join(", ")}) VALUES (${tagPlaceholders})`,
      values,
    );
  }
  ...
  for (const r of eventRows) {
    await target.run(
      "INSERT INTO events (id, ts, action, epc, item_type, bol_number, building, vendor, detail) " +
        "VALUES (?,?,?,?,?,?,?,?,?)",
      [r.id, r.ts, r.action, r.epc, r.item_type, r.bol_number, r.building, r.vendor, r.detail],
    );
  }
  ...
  for (const r of vendorRows) {
    await target.run("INSERT OR IGNORE INTO vendors (name) VALUES (?)", [r.name]);
  }
  ...
  for (const r of noteRows) {
    await target.run(
      "INSERT INTO notes (id, ts, item_type, bol_number, building, text) VALUES (?,?,?,?,?,?)",
      [r.id, r.ts, r.item_type, r.bol_number, r.building, r.text],
    );
  }
  ...
  for (const r of bolRows) {
    await target.run(
      "INSERT INTO bol_docs (id, bol_number, filename, source, pages, vendor, po_number, " +
        "ocr_text, line_items, auto_named, created_at, storage_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
      [
        r.id,
        r.bol_number,
        r.filename,
  ...
  const reqPlaceholders = reqColumns.map(() => "?").join(",");
  const reqRows = legacy.prepare("SELECT * FROM requests ORDER BY id").all() as Record<string, unknown>[];
  for (const r of reqRows) {
    const values = [
      r.id, r.item_type, r.item_name, r.quantity, r.building, r.jobsite, r.requester,
      r.contact, r.note, r.status, r.created_at, r.handled_at, r.handler_note,
      r.order_ref, "",
    ];
    await target.run(
      `INSERT INTO requests (${reqColumns.join(", ")}) VALUES (${reqPlaceholders})`,
      values,
    );
  }
